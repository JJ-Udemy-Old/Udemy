...
function NewFactForm({setFacts, setShowForm}) {
  ...
  async function handleSubmit(e) {
    ...
    if(text && isValidHttpUrl(source) && category && textLength <= 200) console.log("there is data");

    // 3. Creat a new object
    // const newFact = {
...
function CategoryFilter({setCurrentCategory}) {
  ...
  <button className="btn btn-all-categories" onClick={()=>setCurrentCategory("all")}>All
  </button>
  </li>
...
function Fact({ fact, setFacts }) {
  ...
  <div className="vote-buttons">
      <button onClick={() => handelVote ("votesInteresting")} disabled = {isUpdating}>👍 {fact.votesInteresting}</button>
      <button onClick={() => handelVote ("votesMindblowing")} disabled = {isUpdating}>🤯 {fact.votesMindblowing}</button>
      <button onClick={() => handelVote ("votesFalse")} disabled = {isUpdating}>⛔️ {fact.votesFalse}</button>
  </div>
</li>
);
}
